...
class RejectedRequest extends ConsumerStatefulWidget {
  const RejectedRequest({super.key});

  @override
  ConsumerState<RejectedRequest> createState() => _RejectedRequestState();
}

class _RejectedRequestState extends ConsumerState<RejectedRequest> {
  double page = 30;
  bool loading = false;
    ...
    return data.when(
        ...
        data: (info) {
          ...
          final approvedRequests =
              data?.where((request) => request.status == 'declined').toList();
